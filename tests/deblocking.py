...
def deblock_luma_edge(
    ...
    for i in range(FILTER_STEP):
        ...
        if use_strong:
            ...
            # [FIX 1] Luma dùng hệ số 9 và 3 (Không dùng hệ số 4 của Chroma)
            delta = _clip3(-tc, tc, (9 * (q[0] - p[0]) - 3 * (q[1] - p[1]) + 8) >> 4)
            p0_f  = _clip3(0, 255, p[0] + delta)
            q0_f  = _clip3(0, 255, q[0] - delta)

            # Optional p1/q1 update (luma only when dp/dq < β/4)
            tc_div2 = tc >> 1
            # [FIX 2] Phải đảm bảo len >= 3 để gọi được p[2], và phải cộng thêm delta
            if dp < (beta >> 4) and len(p) >= 3:
                dp1 = _clip3(-tc_div2, tc_div2, (((p[2] + p[0] + 1) >> 1) - p[1] + delta) >> 1)
                p1_f = _clip3(0, 255, p[1] + dp1)
            else:
                p1_f = p[1]
                
            if dq < (beta >> 4) and len(q) >= 3:
                # Lưu ý: q1 nằm bên phải nên bị trừ đi delta
                dq1 = _clip3(-tc_div2, tc_div2, (((q[2] + q[0] + 1) >> 1) - q[1] - delta) >> 1)
                q1_f = _clip3(0, 255, q[1] + dq1)
            else:
                q1_f = q[1]
