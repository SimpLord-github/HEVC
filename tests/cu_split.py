...
# NOTE: Rectangular PUs (PART_2NxN, PART_Nx2N) are excluded from the golden model
# because all core kernels (intra_estimation, motion_estimation, mode_decision)
# are designed strictly for SQUARE blocks. Rectangular PUs would require
# non-square block support throughout the entire prediction/transform pipeline.
_INTRA_PART_MODES = [PartMode.PART_2Nx2N]

_INTER_PART_MODES_LARGE = [
    PartMode.PART_2Nx2N,
    # PART_2NxN and PART_Nx2N intentionally excluded: golden model = square blocks only
]
_INTER_PART_MODES_8x8 = [
    PartMode.PART_2Nx2N,
    PartMode.PART_NxN,    # 4 equal 4×4 square sub-blocks — valid for square pipeline
]
_INTER_PART_MODES_4x4 = [PartMode.PART_2Nx2N]
...
def split_ctu(
    ...
    def _recurse(node: QuadNode) -> float:
        ...
        # [FIX] Snapshot immediately — captures all computed fields (pred, residual, etc.)
        # so we can restore without re-running the expensive encode.
        leaf_snapshot = _snapshot(node)

        # ── Step 2: Temporarily split to evaluate children ─────────────
        ...
        for child in children:
            j_split += _recurse(child)

        # ── Step 3: Pick winner ────────────────────────────────────────
        if j_split < j_leaf:
            # Keep split — child decisions are already written
            node.rd_cost = j_split
        else:
            # [FIX] Restore from snapshot — NO re-encoding, O(1) restore
            _restore(node, leaf_snapshot)
            node.rd_cost = j_leaf
